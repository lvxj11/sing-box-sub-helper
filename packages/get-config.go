...
func readIniConfig(settingsPath string, config Settings) (Settings, error) {
	...
	for _, line := range lines {
		...
		case "excludeKeywords":
			config.Filter = []Filter{{Action: "exclude", Keywords: []string{value}}}
		case "outputPath":
			if isValidPath(value) {
				config.OutputPath = value
			}
		case "startStep":
			config.StartStep = strToInt(value)
		}
	}
	return config, nil
}
...
func writeIniConfig(settingsPath string, config Settings) error {
	...
	settingsContent += "; 4.读取临时json，后续步骤由于会缺少数据不可选取。\r\n"
